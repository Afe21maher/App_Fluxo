...
export class EVVMFisher {
    constructor() {
        this.provider = null;
        this.wallet = null;
        this.mateProtocol = null;
        this.pendingTransactions = new Map();
        this.fishingSpots = new Set();
    ...
    async initialize() {
        try {
            ...
            if (config.evvm.sepoliaPrivateKey) {
                ...
                showEVVMBanner();
                logger.info(`EVVM Fisher initialized with wallet: ${this.wallet.address}`);
                // Initialize MATE Protocol
                try {
                    this.mateProtocol = new MATEProtocol(this.provider, this.wallet, config.evvm.mateProtocolAddress);
                    if (this.mateProtocol.isConfigured()) {
                        logger.info(`MATE Protocol initialized: ${this.mateProtocol.getProtocolAddress()}`);
                        ...
                        try {
                            ...
                            const isAvailable = await this.mateProtocol.isFishingSpotAvailable(fishingSpotAddress);
                            if (!isAvailable) {
                        ...
                        logger.warn("MATE Protocol address not configured, using simulation mode");
    ...
    async captureTransaction(tx) {
        try {
            ...
            logger.info(`Transaction ${tx.id} captured and ready for EVVM execution (nonce: ${nonce}, fishing spot: ${fishingSpot})`);
            return tx.id;
        }
        catch (error) {
            ...
            if (this.mateProtocol && this.mateProtocol.isConfigured()) {
                ...
                try {
                    ...
                    try {
                        ...
                        if (receipt) {
                            logger.info(`Transaction ${txId} confirmed in block ${receipt.blockNumber}`);
                        }
                    }
                    catch (waitError) {
                ...
                logger.warn(`MATE Protocol not configured, simulating execution for transaction ${txId}`);
                // Simulate delay
...
//# sourceMappingURL=fisher.js.map