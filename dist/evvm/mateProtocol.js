...
export class MATEProtocol {
    constructor(_provider, wallet, protocolAddress) {
        this.contract = null;
        ...
        if (this.protocolAddress !== DEFAULT_MATE_PROTOCOL_ADDRESS) {
            this.contract = new ethers.Contract(this.protocolAddress, MATE_PROTOCOL_ABI, wallet);
        }
    }
    /**
     ...
     */
    async executeTransaction(evvmTx) {
        if (!this.contract) {
        ...
        try {
            ...
            logger.info(`Executing MATE transaction: ${evvmTx.from} -> ${to}, amount: ${value}, nonce: ${nonce}`);
            // Execute transaction through MATE
            const tx = await this.contract.executeTransaction(to, value, data, nonce, {
     ...
     */
    async isFishingSpotAvailable(spot) {
        if (!this.contract) {
            return false;
        }
        try {
     ...
     */
    async validateTransactionSignature(evvmTx) {
        try {
            // MATE uses EIP-191 signatures
            // Create the message hash using keccak256 of packed data
            const messageHash = ethers.keccak256(ethers.solidityPacked(["address", "address", "uint256", "uint256"], [evvmTx.from, evvmTx.to, evvmTx.amount, evvmTx.nonce]));
...
//# sourceMappingURL=mateProtocol.js.map